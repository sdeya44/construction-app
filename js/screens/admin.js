...
export function renderAdmin() {
  ...
  `;
  document.getElementById('atab-users').addEventListener('click', () => {
    _adminTab = 'users';
    document.querySelectorAll('#admin-tabs .tab').forEach(b => b.classList.remove('active'));
    document.getElementById('atab-users').classList.add('active');
    renderAdminTabContent();
  });
  document.getElementById('atab-assignments').addEventListener('click', () => {
    _adminTab = 'assignments';
    document.querySelectorAll('#admin-tabs .tab').forEach(b => b.classList.remove('active'));
    document.getElementById('atab-assignments').classList.add('active');
    renderAdminTabContent();
  });
  renderAdminTabContent();
}
...
function renderAdminTabContent() {
  ...
  if (!el) return;
  if (_adminTab === 'users') renderUsersTab(el);
  else renderAssignmentsTab(el);
}
...
function renderUsersTab(el) {
  ...
  const usersHtml = D.users.length
    ? D.users.map(u => {
        const isMe = D.user && D.user.email === u.email;
        const initial = (u.name || u.email || '?')[0].toUpperCase();
        const isGM = u.role === 'GeneralManager';
        const roleLabelHe = isGM ? 'מנהל כללי' : 'מנהל אתר';
        const badgeClass = isGM ? 'b-red' : 'b-blue';
        const toggleRoleLabel = isGM ? 'שנה ל-מנהל אתר' : 'שנה ל-מנהל כללי';
        return `<div class="list-item" data-uid="${u.id}">
          ...
          <span class="badge ${badgeClass}">${roleLabelHe}</span>
          <div class="btn-row" style="margin-right:8px;flex-shrink:0">
            <button class="btn btn-outline btn-sm btn-toggle-role" data-uid="${u.id}">${toggleRoleLabel}</button>
            <button class="btn btn-danger btn-sm btn-del-user" data-uid="${u.id}" ${isMe ? 'disabled title="לא ניתן למחוק את עצמך"' : ''}>מחק</button>
          </div>
        </div>`;
      }).join('')
  ...
  el.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="form-group">
        ...
        <input class="form-input" id="new-user-email" type="email" placeholder="user@example.com" dir="ltr">
      </div>
      <div class="form-group">
        <label class="form-label">שם</label>
        <input class="form-input" id="new-user-name" type="text" placeholder="שם מלא">
      </div>
      <div class="form-group">
  ...
  el.querySelectorAll('.btn-toggle-role').forEach(btn => {
    btn.addEventListener('click', () => toggleUserRole(btn.dataset.uid));
  });
...
async function deleteUser(id) {
  ...
  if (!user) return;
  if (D.user && D.user.email === user.email) { toast('לא ניתן למחוק את עצמך', 'err'); return; }
...
async function addUser() {
  ...
  const role    = roleEl?.value || 'SiteManager';
  if (!email) { toast('יש להזין אימייל', 'err'); return; }
  if (!name)  { toast('יש להזין שם',    'err'); return; }
  if (D.users.find(u => u.email.toLowerCase() === email)) {
    toast('משתמש עם אימייל זה כבר קיים', 'err');
    return;
  }
  const id      = uid();
  const addedAt = new Date().toISOString();
  const addedBy = D.user?.email || '';
  const newUser = { id, email, name, role, addedAt, addedBy };
...
function renderAssignmentsTab(el) {
  ...
  const currentSel = selValue && siteManagers.find(u => u.email === selValue)
    ? selValue
    : siteManagers[0].email;

  el.innerHTML = `
    <div class="card" style="margin-bottom:12px">
      <div class="form-group">
        <label class="form-label">מנהל אתר</label>
...
function renderAssignmentsPanel() {
  ...
  if (!panel) return;
  const selEl = document.getElementById('assign-user-sel');
  const email = selEl?.value || '';
  if (!email) return;

  const existing = D.siteAssignments
    ? D.siteAssignments.filter(a => a.email === email)
    : [];

  const assignedSiteIds = new Set(existing.map(a => a.siteId));
  const availableSites  = D.sites.filter(s => !assignedSiteIds.has(s.id));
  ...
  panel.querySelectorAll('.btn-remove-assign').forEach(btn => {
    btn.addEventListener('click', () => removeAssignment(btn.dataset.aid));
  });
  const addBtn = document.getElementById('btn-add-assign');
  if (addBtn) addBtn.addEventListener('click', addAssignment);
}

async function addAssignment() {
  if (!can('manage_site_assignments')) { toast('אין הרשאה', 'err'); return; }
  const selEl   = document.getElementById('assign-user-sel');
  const siteEl  = document.getElementById('assign-site-sel');
  const email   = selEl?.value || '';
  ...
  if (!D.siteAssignments) D.siteAssignments = [];
  if (D.siteAssignments.find(a => a.email === email && a.siteId === siteId)) {
    toast('הקצאה זו כבר קיימת', 'err');
    return;
  }
  ...
  if (document.getElementById('btn-add-assign')) {
...
async function removeAssignment(aid) {
  if (!can('manage_site_assignments')) { toast('אין הרשאה', 'err'); return; }
  if (!D.siteAssignments) return;
  const rec = D.siteAssignments.find(a => a.id === aid);
  if (!rec) return;
  const prev = [...D.siteAssignments];
  D.siteAssignments = D.siteAssignments.filter(a => a.id !== aid);
  try {
    await rebuildTab('SiteAssignments', D.siteAssignments.map(a => [a.id, a.email, a.siteId, a.siteName, a.addedAt, a.addedBy]));
    await logAudit('DELETE', 'SiteAssignment', aid, `הסרת הקצאת אתר ${rec.siteName} ממנהל ${rec.email}`);
    toast('הקצאה הוסרה ✓', 'ok');
  } catch (e) {
    D.siteAssignments = prev;
    toast('שגיאה: ' + e.message, 'err');
  }
  renderAssignmentsPanel();
}
