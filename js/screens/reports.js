...
export function exportSiteMonthPDF(siteId, month, year) {
  ...
  const html = buildPrintDoc(`יומן ביצוע חודשי — ${site.name}`, `${MN[month]} ${year} | הופק: ${new Date().toLocaleDateString('he-IL')}`,
    `<table><thead><tr><th>תאריך</th><th>פעילויות</th><th>עובדים</th><th>ציוד</th><th>אספקות</th><th>הערות</th></tr></thead>
     <tbody>${rows}</tbody>
     <tfoot><tr style="background:#4f46e5"><td colspan="6" style="font-weight:800;color:#fff">סה"כ ${siteLogs.length} ימי עבודה | ${MN[month]} ${year}</td></tr></tfoot></table>`);
...
export function exportAllEmployeesPDF() {
  ...
  emps.forEach(([,emp], idx) => {
    ...
    for (let d=1; d<=daysCount; d++) {
      ...
      dayHeaders += `<th style="min-width:16px;${wknd?'background:#3730a3':''}">${d}<br><span style="font-size:8px">${DAYS_HE[dow]}</span></th>`;
      const worked = emp.dates.has(dateStr); if (worked) total++;
      dayCells += `<td style="${wknd?'background:#e0e7ff':''}${worked?'color:#4f46e5;font-weight:800;font-size:14px':''}">${worked?'✓':''}</td>`;
    }
    pages += `<div class="page"${idx>0?' style="page-break-before:always"':''}>
      ...
      <tbody><tr><td style="font-weight:700;background:#eef2ff;text-align:right">${emp.name}</td>${dayCells}<td class="tot-c">${total}</td></tr></tbody></table>
  ...
  const html = `<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="UTF-8">
    ...
    <style>*{font-family:'Heebo',sans-serif;box-sizing:border-box;margin:0;padding:0}body{direction:rtl;font-size:11px;background:#fff}
    .biz-hdr{color:#4f46e5;font-size:12px;font-weight:800;text-align:center;padding:6px 0 2px}
    .page{padding:6mm}.hdr{background:linear-gradient(135deg,#4f46e5,#6366f1);color:#fff;padding:10px 14px;border-radius:8px;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between}
    .emp-name{font-size:16px;font-weight:800;color:#fff}.sub2{font-size:12px;color:rgba(255,255,255,.8)}
    table{width:100%;border-collapse:collapse;font-size:10px}th{background:#4f46e5;color:#fff;padding:3px 1px;text-align:center;font-size:9px}
    td{padding:4px 1px;border:1px solid rgba(99,102,241,.12);text-align:center}.tot-h{background:#6366f1;color:#fff;font-weight:800}
    .tot-c{background:#6366f1;color:#fff;font-weight:800;font-size:13px}.footer{text-align:center;color:#888;font-size:10px;margin-top:6px}
...
function buildPrintDoc(title, subtitle, body) {
  return `<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="UTF-8">
    ...
    .biz-name{color:#4f46e5;font-size:13px;font-weight:800;text-align:center;margin-bottom:2px}
    h2{color:#4f46e5;text-align:center;font-size:18px;margin-bottom:4px}.sub{color:#666;text-align:center;font-size:12px;margin-bottom:16px}
    table{width:100%;border-collapse:collapse}th{background:#4f46e5;color:#fff;padding:8px 6px;font-size:11px;text-align:center}
    td{padding:7px 6px;border-bottom:1px solid rgba(99,102,241,.12);font-size:11px;text-align:center;vertical-align:top}
    tr:nth-child(even)td{background:#f5f3ff}@media print{body{margin:8px}}</style></head><body>
